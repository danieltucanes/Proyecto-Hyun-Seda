...
public class GUIUserOption extends javax.swing.JFrame {
     ...
     */
    public GUIUserOption(int opt,String token) {
        this.opt = opt;
        initComponents();
        this.token=token;
       ...
       llenarListaRoles(token);
    }
    
    public GUIUserOption(int opt, Long userId,String token) {
        //////////////////////////////////////HEY
     ...
     public GUIUserOption(String token) {
        //////////////////////////////////////HEY
        //SE PUEDEN PONER LA INFO DEL USUARIO QUE ESTABA ANTES PARA EDITAR
    //    this.opt = opt;
      //  this.userId = userId;
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
      ...
      //   System.out.println(new BCryptPasswordEncoder().encode("1234"));
        ...
        System.out.println("imprimiendo rol seleccioando "+ listRoles.getSelectedValuesList().get(0));
        User user = new User();
        ...
        // Obtener los roles seleccionados y convertirlos a una lista de Role
    List<Role> roles = new ArrayList<>();
//    for (Object selectedValue : listRoles.getSelectedValuesList()) {
//    if (selectedValue instanceof Role) {
//        roles.add((Role) selectedValue);
//    }
//        
//}
...
String roleName = listRoles.getSelectedValuesList().get(0);

// Crear un objeto Role utilizando el nombre del rol
Role selectedRole = new Role();
selectedRole.setRoleName(roleName);

// Crear una lista de roles y agregar el objeto Role creado
List<Role> selectedRoles = new ArrayList<>();
selectedRoles.add(selectedRole);

// Ahora puedes usar selectedRoles como una lista de roles

        
        
       // List<Role> roles = (List<Role>) listRoles;
        user.setUserRoles(selectedRoles);
       // user.setUserRoles(sacarObjRolesSelecionados());
        
        if(opt == 0){
            System.out.println("creando USARIO POR ADMINISTRADOR");
            service.create(user,token);
            
            Messages.showMessageDialog("Se ha creado el usuario correctamente", "Confirmación");
            
        }else if(opt == 1){
            
            service.edit(userId, user,token);
    ...
    private void llenarListaRoles(String token){
        System.out.println("esta en el metodo donde busca los roles");
        System.out.println("roles "+ rolService.findAll(token));
        
        List<Role> roles = rolService.findAll(token);
        
         System.out.println("paso ahora tienes un arreglo de roles");
        
        listRoles.setModel(model);
        for (Role role : roles) {
            if(!role.getRoleName().equals("SUPERADMIN") && !role.getRoleName().equals("USERL")){
    ...
    private void ponerInterfazAnterior(){
        ...
        GUIUser guiUser = new GUIUser(token);
